...
class Box:
    ...
    def generic_coords(self, pt):
        u = lerp(pt[0], self.planes[0][0], self.planes[0][1], -1, 1)
        v = lerp(pt[1], self.planes[1][0], self.planes[1][1], -1, 1)
        n = lerp(pt[2], self.planes[2][0], self.planes[2][1], -1, 1)
        return u,v,n
...
class Sphere:
    ...
    def generic_coords(self, pt):
        u = lerp(pt[0], self.pos[0]-self.radius, self.pos[0]+self.radius, -1, 1)
        v = lerp(pt[1], self.pos[1]-self.radius, self.pos[1]+self.radius, -1, 1)
        n = lerp(pt[2], self.pos[2]-self.radius, self.pos[2]+self.radius, -1, 1)
        return u,v,n
    ...
    def _setinfo(self, ray, t, info):
        ...
        info.uvn = self.generic_coords(p)
        info.texture = self.texture
